...
def ipw_downsampling(df: pd.DataFrame,
                     K_IPW_RATIO=7, ipw_propensity_cols_list: [] = ['age',
                                                                    'Sex',
                                                                    'age_X_sex',
...
def make_target_df( DIAG_TIDY_PATH="ukbb-hack/df_diag_tidy.parquet",  # ../../
        ...
        , FAST_SAMPLE_SIZE=30_000  # 60_000
    ...
    # global K_diag_thresh_value, CORR_THRESH, do_stat_fs_filt, do_boruta_fs, do_mi_fs_filt, USE_CAT_COLS, FAST,  OUTPUT_FEATURE_REPORT_NAME, DO_CV, DROP_FEAT_COLS_LIST, df, c
    # In[5]:
    ...
    df_meta = pd.read_parquet("df_ukbb_aux_tidy.parquet", columns=["Date of attending assessment centre", "eid"]).set_index("eid")
    ...
    # category_number
    df_phecodes_map = df_phecodes_map.merge(pd.read_csv("phecode_definitions1.2.csv.zip", usecols=["phecode",
    ...
    if JOIN_CHOL_FEATS:  ## not just chol feats!!
        ...
        df_aux = pd.read_parquet("df_ukbb_aux_tidy.parquet")
...
def model_features(df: pd.DataFrame, FAST=False
                   ...
                   , USE_CAT_COLS=True
                   , SAVE_OUTPUT=True,
                   K_diag_thresh_value=250
                   , OUTPUT_FEATURE_REPORT_NAME="gallstone_chol_ipw_feature_report.csv",
                   DROP_FEAT_COLS_LIST: [] = ["cutoff_date", "prediction_cutoff_year",
                                              'PRS genetic principal components | Array 0',
                                              'PRS genetic principal components | Array 1',
                                              'PRS genetic principal components | Array 2',
                                              'PRS genetic principal components | Array 3',
                                              'PRS genetic principal components | Array 4', ]
                   , do_shap = True, return_res:bool=False
                   ):
    if not USE_CAT_COLS:
    ...
    cfs = SmartCorrelatedSelection(
        threshold=min(1.13 * CORR_THRESH, 0.992),
        # method=dcor.distance_correlation ## slower but more flexible, and leaves more cases
        # ,method="kendall",
        # ,method= "spearman",
        ).fit(OrdinalEncoderPandas(return_pandas_categorical=True).fit_transform(X).sample(frac=0.6))  #
    ...
    feature_importance = make_feature_importance_utility_selection(X, y, pipeline, transformed_feature_names, shap_values, SAVE_OUTPUT,
                                                                   OUTPUT_FEATURE_REPORT_NAME)
    if return_res:
        res_dict = {"feature_importance_df":feature_importance,"pipeline":pipeline,"feature_names":transformed_feature_names}
        return res_dict
...
def make_feature_importance_utility_selection(X, y, pipeline, feature_names:[], shap_values, SAVE_OUTPUT=False, OUTPUT_FEATURE_REPORT_NAME="feature_importance.csv"):
    ...
    #     X_trans = SmartCorrelatedSelection(threshold= 0.85,selection_method = "model_performance",
    #                                # method=dcor.distance_correlation ## slower but more flexible, and leaves more cases
    #                                method=  "spearman", #"kendall",
...
def convert_ukbb_df_to_x_y(df, DROP_FEAT_COLS_LIST=[], K_diag_thresh_value=200, to_drop_text_cols_list=[],
    ...
    X = df.drop(columns=["y", "eid", "YOB", "prediction_cutoff_year"] + to_drop_text_cols_list  # +DROP_FEAT_COLS_LIST - done after
                , errors="ignore").dropna(axis=1, thresh=20 + (K_diag_thresh_value // 5)).copy()
...
if __name__ == "__main__":
    ...
    res_dict = model_features(df=df, FAST=Fast_Run,do_boruta_fs=False, SAVE_OUTPUT = True,
                   OUTPUT_FEATURE_REPORT_NAME = "gallstone_ipw_broad_feature_report.csv",)
